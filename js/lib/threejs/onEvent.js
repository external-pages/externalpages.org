;(function (definition) {
  ...
  // attach to 3d not window
  const threeCanvas = document.getElementById('ThreeJS').children
  ...
  listenerList.click = function (targetList, camera) {
    ...
    threeCanvas[0].addEventListener('mousedown', down, false)
    threeCanvas[0].addEventListener('mousemove', move, false)
    threeCanvas[0].addEventListener('mouseup', up, false)
  }
  ...
  listenerList.hover = function (targetList, camera) {
    var targetObject,
      ...
      Hover = false
    var Mouse = new THREE.Raycaster()
    threeCanvas[0].addEventListener(
      'mousemove',
      function (event) {
        event.preventDefault()
        if (!targetList) return
